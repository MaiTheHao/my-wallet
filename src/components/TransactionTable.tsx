...
export function TransactionTable() {
  ...
  const toggleSelectAll = () => {
    setSelectedIds(
      selectedIds.length === transactions.length ? [] : transactions.map((t) => t._id),
    );
  };
            ...
            <table className='w-full border-collapse'>
              <thead>
                <tr className='border-b border-gray-200'>
                  ...
                  <th className='px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider w-32'>
                    Thời gian
                  </th>
                  <th className='px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider'>
                    Nội dung
                  </th>
                  <th className='px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider w-40'>
                    Danh mục
                  </th>
                  <th className='px-4 py-3 text-right text-xs font-bold text-gray-900 uppercase tracking-wider w-40'>
                    Số tiền (VNĐ)
                  </th>
                ...
                {transactions.map((transaction) => (
                  ...
                  >
                    ...
                    <td className='px-4 py-3 text-right whitespace-nowrap'>
                      <span
                        className={`text-sm font-mono font-medium tracking-tight ${transaction.type === 'income' ? 'text-green-700' : 'text-gray-900'}`}
                      >
            ...
            {transactions.map((transaction) => (
              ...
              >
                ...
                <div className='shrink-0 ml-4 text-right'>
                  <span
                    className={`block font-mono font-medium text-sm ${transaction.type === 'income' ? 'text-green-700' : 'text-gray-900'}`}
                  >
                    {formatAmount(transaction.amount, transaction.type)}
                  </span>
                </div>
