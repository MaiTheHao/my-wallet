...
export const ChatInterface = React.memo(function ChatInterface() {
	...
	return (
		<Section icon={<Sparkles className='text-black' size={18} />} title='Trợ lý AI' subtitle='Nhập chi tiêu bằng ngôn ngữ tự nhiên' id='chat-interface'>
			<div className='flex flex-col h-full'>
				<div className='min-h-[300px] max-h-[400px] overflow-y-auto px-1 py-4 flex flex-col gap-3'>
					{chatHistory.length === 0 ? (
						<div className='flex flex-col items-center justify-center h-full text-gray-400 my-auto py-8'>
							...
							<p className='text-xs text-gray-500 mt-1'>Thử nhập: "Ăn sáng 30k", "Lương 15tr"</p>
						</div>
					) : (
						<>
							{chatHistory.map((chat, index) => (
								<div
									key={index}
									className={`flex ${chat.type === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}
								>
									<div
										className={`max-w-[85%] px-4 py-3 text-sm leading-relaxed shadow-sm rounded-2xl ${
											chat.error
				...
				<div className='pt-4 mt-2 border-t border-gray-100'>
					<form onSubmit={handleSubmitChat} className='relative flex items-center gap-2'>
						<input
							className='w-full pl-4 pr-12 py-3.5 bg-gray-50 border-none rounded-lg text-gray-900 text-sm placeholder:text-gray-400 focus:bg-white focus:ring-2 focus:ring-black/5 transition-all duration-200'
							value={message}
